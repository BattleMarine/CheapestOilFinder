...
- 현재 위치 화면과 목적지 화면의 GPS 버튼은 기기 위치를 읽어 지도 중심을 갱신하고 현재 위치 마커를 표시합니다.
...
- 현재 위치 화면의 GPS 갱신은 위치가 잡히는 즉시 현위치 마커를 먼저 표시하고, 이후 백엔드 주유소를 다시 조회합니다.
- 현재 위치와 주유소 마커는 런타임에 생성한 비트맵 스타일로 그리며, Kakao Map LabelLayer에 개별 라벨로 추가합니다.
- 현재 위치와 주유소 마커는 각각 별도 `LabelLayer`를 사용하고, `setVisible(true)`와 `showAllLabel()`로 숨김 상태를 방지합니다.
